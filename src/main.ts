...
export default class DendronBridgePlugin extends Plugin {
  ...
  async onload() {
    ...
    this.workspace = new DendronBridgeWorkspace(this.app, this.settings);

    addIcon(dendronBridgeActivityBarName, dendronBridgeActivityBarIcon);
    ...
    this.addCommand({
      id: "open-dendron-bridge",
      name: "Open Tree",
      callback: () => this.activateView(),
    });
    this.addCommand(exportNotesCommand(this.app, this.workspace, this.settings));

    this.addSettingTab(new DendronBridgeSettingTab(this.app, this));

    this.registerView(VIEW_TYPE_DENDRON_BRIDGE, (leaf) => new DendronBridgeView(leaf, this));

    this.addRibbonIcon(this.settings.pluginIcon, "Open Dendron Bridge", () => {
      this.activateView();
  ...
  updateNoteStore() {
    dendronBridgeVaultList.set(this.workspace.vaultList);
  }
  ...
  onFileMenu = (menu: Menu, file: TAbstractFile) => {
    ...
    menu.addItem((item) => {
      item
        ...
        .setTitle("Reveal in Dendron Bridge")
  ...
  async saveSettings() {
    ...
    this.app.workspace.trigger("dendron-bridge:settings-changed");
  }
  ...
  updateViewLeafIcon() {
    let leaves = this.app.workspace.getLeavesOfType(VIEW_TYPE_DENDRON_BRIDGE);
    if (leaves.length == 0) {
      return;
    }
