...
function getGlobalNodes(
  ...
  const dendronBridgeNodeList: DendronBridgeGraphNode[] = workspace.vaultList.flatMap((vault) =>
    vault.tree
      .flatten()
  ...
  if (progression) {
    const map = new Map<DendronBridgeGraphNode, number>();
    for (const dendronBridgeNode of dendronBridgeNodeList) {
      if (dendronBridgeNode.type === "file") {
        map.set(
          dendronBridgeNode,
          Math.min(dendronBridgeNode.file.stat.ctime, dendronBridgeNode.file.stat.mtime)
        );
        continue;
      } else if (dendronBridgeNode.type === "note") {
        const file = dendronBridgeNode.note.file;
        if (!file) {
          map.set(dendronBridgeNode, Infinity);
        } else {
          const metadata = app.metadataCache.getFileCache(file)?.frontmatter;

          if (!metadata) map.set(dendronBridgeNode, Infinity);
          else {
            const created = parseInt(metadata["created"]);
            map.set(dendronBridgeNode, isNaN(created) ? Infinity : created);
          }
        }
      }
    }
    dendronBridgeNodeList.sort((a, b) => map.get(a)! - map.get(b)!);
  }

  let stopFile: TFile | undefined = undefined;
  for (const dendronBridgeNode of dendronBridgeNodeList) {
    if (dendronBridgeNode.type === "note") {
      const { note, vault } = dendronBridgeNode;
      ...
      nodes[`dendron-bridge://${vault.config.name}/${note.getPath()}`] = node;
      ...
      for (const link of listOfLinks) {
        ...
        if (target?.type === "maybe-note") {
          const linkName = `dendron-bridge://${target.vaultName}/${target.path}`.toLowerCase();
    ...
    } else if (dendronBridgeNode.type === "file") {
      const linkName = dendronBridgeNode.file.path;
      if (options.showOrphans) {
        if (progression && progression === numLinks) {
          stopFile = dendronBridgeNode.file;
        }
        numLinks++;
      }
  ...
  if (progression) {
    const index = dendronBridgeNodeList.findIndex(({ file }) => file === stopFile);

    if (index >= 0) {
      for (let i = index + 1; i < dendronBridgeNodeList.length; i++) {
        const dendronBridgeNode = dendronBridgeNodeList[i];

        const p =
          dendronBridgeNode.type === "note"
            ? `dendron-bridge://${dendronBridgeNode.vault.config.name}/${dendronBridgeNode.note.getPath()}`
            : dendronBridgeNode.file.path;
        if (!nodes[p]) {
...
function getLocalNodes(
  ...
  const localFileDPath = `dendron-bridge://${vault.config.name}/${file.basename}`;
  ...
  if (isTopLevel) {
    const rootPath = `dendron-bridge://${vault.config.name}/${vault.tree.root.getPath()}`;
    addNode(rootPath);
  ...
  vault.tree.flatten().forEach((note) => {
    ...
    const notePath = `dendron-bridge://${vault.config.name}/${note.getPath()}`;
...
export function createDataEngineRender(
  ...
  return function (this: GraphEngine) {
    const nodes: Record<string, DendronBridgeGraphNode> = {};
