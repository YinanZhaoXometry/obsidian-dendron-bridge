...
export function openParentNoteCommand(app: App, workspace: DendronBridgeWorkspace) {
  return {
    id: "open-parent-note",
...
async function openParentNote(app: App, workspace: DendronBridgeWorkspace) {
  ...
  if (!vault) {
    new Notice("File is not in a Dendron Bridge vault");
    return;
  }

  const note = vault.tree.getFromFileName(activeFile.basename, workspace.settings);
  if (!note) {
    new Notice("Cannot find note in Dendron Bridge tree");
    return;
  }
