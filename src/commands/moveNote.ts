...
export function moveNoteCommand(app: App, workspace: DendronBridgeWorkspace) {
  return {
    id: "dendron-bridge-move-file",
...
export async function moveNoteToVault(app: App, workspace: DendronBridgeWorkspace, file?: TFile) {
  ...
  if (!sourceVault) {
    new Notice("Current file is not in a Dendron Bridge vault");
    return;
  }
