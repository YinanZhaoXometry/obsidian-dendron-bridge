...
export class DendronBridgeWorkspace {
  ...
  findVaultByParentPath(path: string): DendronBridgeVault | undefined {
  ...
  resolveRef(sourcePath: string, link: string): RefTarget | null {
    if (link.startsWith(DENDRON_BRIDGE_URI_START)) {
      const [vaultName, rest] = link.slice(DENDRON_BRIDGE_URI_START.length).split("/", 2) as (
        | string
        | undefined
      )[];
