...
export class RenameNoteModal extends Modal {
  ...
  constructor(
    ...
    super(app);
    this.modalEl.addClass("dendron-rename-modal");
  }

  onOpen() {
    const { contentEl } = this;
    contentEl.addClass("dendron-rename-modal");

    contentEl.createEl("h2", { text: "Rename Note" });
    contentEl.createEl("h5", { text: `${this.file.basename}` });

    const inputContainer = contentEl.createDiv("dendron-rename-input-container");

    this.newNameInput = inputContainer.createEl("input", {
      type: "text",
      value: this.file.basename,
      cls: "dendron-rename-input",
