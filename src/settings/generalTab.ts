...
export function renderGeneralTab(tab: DendronBridgeSettingTab, containerEl: HTMLElement) {
  ...
  new Setting(containerEl)
    ...
    .setDesc(
      "Single click in the tree preview files in a temporary tab. " +
      "Double click or editing promotes to a permanent tab."
    )
    .addToggle((toggle) => {
  ...
  new Setting(containerEl)
    ...
    .addToggle((toggle) => {
      toggle
        .setValue(tab.plugin.settings.previewTabsAutoPromote)
        .onChange(async (value) => {
